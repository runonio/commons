			...
			for(Map<String, Object> data : dataList){
				for(i=0 ; i<nameClassArray.length ; i++){
					...
					try{
						...
						}else if(nameClassArray[i].getClasses()  == Boolean.class){
						...
						if(objClasses == Timestamp.class){
							...
							pstmt.setDouble(i+1, (Double)object);
						}else if(objClasses == Boolean.class){
							pstmt.setBoolean(i +1, (Boolean)object);
						} else{
							pstmt.setString(i+1, (String)object);
						}
					...
					try{
						...
						if(dataType == JdbcDataType.STRING){
							...
							if(result.wasNull()){
								...
								data.put(columnName, result.getLong(columnName));
							}
							...
							if(result.wasNull()){
								...
								data.put(columnName, result.getDouble(columnName));
							}
							...
							if(result.wasNull()){
								...
								data.put(columnName, result.getBoolean(columnName));
							}
						}
					}catch(Exception e){
						data.put(columnName, result.getString(columnName));
